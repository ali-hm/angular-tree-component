    ...
    this.router.events
      ...
      .subscribe((event: NavigationEnd) => {
        ...
        this.nav.forEach((navNode) => {
          if (
            navNode.link &&
            this.router.isActive(this.router.createUrlTree(navNode.link), false)
          ) {
            ...
            navNode.children.forEach((childNode) => {
              if (
                ...
                this.router.isActive(
                  this.router.createUrlTree(childNode.link),
                  false
                )
              ) {
        ...
        if (
          newCurrent.length >= 2 &&
          newCurrent.find((n) => n.title === this.nav[0].title) !== undefined
        ) {
        ...
        if (newCurrent?.length && newCurrent[0].title !== 'Getting Started') {
          this.titleService.setTitle(
            `${this.baseTitle} - ${newCurrent[0].title}`
          );
        } else {
